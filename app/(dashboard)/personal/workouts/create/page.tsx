...
export default function CreateWorkoutPage() {
    ...
    const tWiz = useTranslations('WorkoutWizard');
    ...
    const handleSave = async () => {
        setIsLoading(true);
        try {
            const personalId = authUser?.id || "00000000-0000-0000-0000-000000000000";
            ...
            if (response.ok) {
                alert(tWiz('save_success'));
                router.push("/personal");
            } else {
                const errorData = await response.json();
                alert(`Error: ${errorData.message}`); // Keep generic error or map it? Keeping for now.
            }
        } catch (err) {
            console.error(err);
            alert(tWiz('save_error'));
        } finally {
            setIsLoading(false);
        }
    };
                ...
                <TabsContent value="details">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>{tWiz('details_desc')}</CardDescription>
                        </CardHeader>
                            ...
                            <div className="grid gap-2">
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        ...
                                        {students.length === 0 && <SelectItem value="none" disabled>{tWiz('no_student')}</SelectItem>}
                                    </SelectContent>
                                </Select>
                            </div>
                        ...
                        <CardFooter>
                            <Button onClick={() => setActiveTab("exercises")} className="ml-auto">{tWiz('continue')}</Button>
                        </CardFooter>
                    </Card>
                </TabsContent>
                    ...
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between">
                            <div>
                                ...
                                <CardDescription>{tWiz('exercises_desc')}</CardDescription>
                            </div>
                        ...
                        <CardContent className="p-0 sm:p-6 space-y-6">
                            <div className="p-3 sm:p-4 border rounded-lg bg-slate-50 dark:bg-slate-900 space-y-4">
                                <div className="flex justify-between items-center">
                                    <h3 className="font-bold text-base md:text-lg">{tWiz('exercises_day_title')} 1: Superiores A</h3>
                        ...
                        <CardFooter className="flex justify-between">
                            <Button variant="ghost" onClick={() => setActiveTab("details")}>{tWiz('back')}</Button>
                            <Button onClick={() => setActiveTab("review")}>{tWiz('continue')}</Button>
                        </CardFooter>
                    </Card>
                </TabsContent>
                ...
                <TabsContent value="review">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>{tWiz('review_desc')}</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="p-4 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
                                ...
                                <div className="text-sm space-y-1 text-slate-600 dark:text-slate-400">
                                    <p>{tWiz('labels.student')}: Alice Johnson</p>
                                    <p>{tWiz('labels.duration')}: 4 semanas</p>
                                    <p>{tWiz('labels.frequency')}: 5 dias por semana</p>
                                    <p>{tWiz('labels.total_exercises')}: {exercises.length}</p>
                                </div>
                            </div>
                        </CardContent>
                        <CardFooter className="flex justify-between">
                            <Button variant="ghost" onClick={() => setActiveTab("exercises")}>{tWiz('back')}</Button>
                            <Button
